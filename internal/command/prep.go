...
func prepItemWriteOnly(s *store.Store, cfg *config.Config, itemID string, item *model.Item, opts PrepOpts, engine RunEngine, worktreeDir string) string {
	...
	// would otherwise be left untracked on disk until the next independent
	// st sync — exactly the race I-982 fixes.
...
func stampPrepSuccess(s *store.Store, itemID string) {
	...
	if err := s.Mutate(itemID, func(it *model.Item) error {
		...
		it.Doc.SetField("plan_written_at", now)
		it.PlanFailedAt = ""
		it.Doc.SetField("plan_failed_at", "")
		it.PlanFailureReason = ""
		it.Doc.SetField("plan_failure_reason", "")
		return nil
	}); err != nil {
		fmt.Fprintf(os.Stderr, "[%s] Warning: could not stamp plan_written_at: %v\n", itemID, err)
	}
}
...
func stampPrepFailure(s *store.Store, itemID, reason string) {
	...
	if err := s.Mutate(itemID, func(it *model.Item) error {
		it.PlanFailedAt = now
		it.Doc.SetField("plan_failed_at", now)
		it.PlanFailureReason = reason
		it.Doc.SetField("plan_failure_reason", reason)
		it.PlanWrittenAt = ""
		it.Doc.SetField("plan_written_at", "")
		return nil
	}); err != nil {
...
func buildPrepPrompt(cfg *config.Config, itemID string, item *model.Item) string {
	...
	b.WriteString("## Tests\n")
	b.WriteString("What tests cover this work? Name specific test files/functions, or write 'None — existing coverage sufficient: <reason>'.\n\n")
	b.WriteString("## Out-of-scope\n")
	b.WriteString("List out-of-scope items as linked issues (I-XXX: description), or write 'None'.\n\n")
	b.WriteString("## Risks\n")
	b.WriteString("List risks and mitigations, or write 'None — low-risk change: <reason>'.\n\n")
	...
	b.WriteString("In `## Files to Create` and `## Files to Modify`, emit bare workspace-relative\n")
	b.WriteString("paths only — no backticks, no parenthesized annotations, no em-dash trailing\n")
	b.WriteString("prose. Descriptions belong in `## Implementation Steps`, not the file lists.\n")
	b.WriteString("Bad: `- ` then backtick then `internal/foo.go` then backtick then ` (adds bar)`. Good: `- internal/foo.go`.\n\n")

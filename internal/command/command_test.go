...
func TestCloseHappy(t *testing.T) {
	s, cfg := setupTestEnv(t)
	code := Close(s, cfg, "T-003", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, })
	if code != 0 {
...
func TestCloseAutoRemovesFromQueue(t *testing.T) {
	...
	if code := Close(s, cfg, "T-003", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, }); code != 0 {

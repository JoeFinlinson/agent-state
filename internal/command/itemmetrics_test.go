...
func TestFormatLinePrefersLOCOverTokens(t *testing.T) {
	// When both LOC and tokens are present, LOC wins (closer to "what
	// shipped"); tokens are consulted only for items with no diff yet.
	...
	if strings.Contains(got, "tok") {
		t.Errorf("FormatLine = %q, should not include tokens when LOC is present", got)
	}
}

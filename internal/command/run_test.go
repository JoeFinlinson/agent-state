...
`)
	...
	s3, _ := store.New(cfg)
	closeCode := Close(s3, cfg, "T-010", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, Force: true})
	if closeCode != 0 {

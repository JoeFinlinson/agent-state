...
func TestCloseRecordsTimeTracking(t *testing.T) {
	...
	// Close
	code := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, Force: true})
	if code != 0 {

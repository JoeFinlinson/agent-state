...
func TestClose_FreezesDurationsAndLOC(t *testing.T) {
	...
	code := Close(env.S, env.Cfg, "T-003", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, 
...
func TestClose_DurationMetricsAgree(t *testing.T) {
	...
	code := Close(env.S, env.Cfg, "T-003", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, Force: true})
	if code != 0 {
		t.Fatalf("Close exit=%d", code)
	}

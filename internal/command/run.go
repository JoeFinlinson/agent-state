...
func executePlanWithOpts(s *store.Store, cfg *config.Config, itemID string, engine RunEngine, opts RunOpts, worktreeDir string) StepResult {
	...
	if needsSummary || needsACs {
		...
		if !plan.Exists(cfg.PlansDir(), itemID) {
			...
			// T-394: parse the Claude output to capture Tests/OutOfScope/Risks so
			// the draft sidecar passes ValidatePlan when st plan approve is called later.
			if parsed, err := plan.Parse(proposalResult.Text); err == nil {
				if parsed.Tests != "" {
					draftPlan.Tests = parsed.Tests
				}
				if parsed.OutOfScope != "" {
					draftPlan.OutOfScope = parsed.OutOfScope
				}
				if parsed.Risks != "" {
					draftPlan.Risks = parsed.Risks
				}
			}
	...
	if len(syntaxErrors) > 0 {
		fmt.Printf("\n⚠ %d AC(s) have shell syntax or anti-pattern errors:\n", len(syntaxErrors))

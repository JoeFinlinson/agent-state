...
func TestReopenClearsCompletionMarkers(t *testing.T) {
	...
	if rc := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, Force: true, Reason: "superseded"}); rc != 0 {
...
func TestReopenWorksForIssue(t *testing.T) {
	...
	if rc := Close(s, cfg, "I-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", SkipAC: "test: AC gate not under test", SkipACRequested: true, Force: true}); rc != 0 {
